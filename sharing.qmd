...
I used [Quarto](https://quarto.org/) to publish my Capstone Project report for the Google Data analytics Certification, containing my methods, analysis code chunks, results including plots, insights and lessons learned. More about the set-up in the next paragraph. Example R Code chunks with my analysis code and outputs can be seen at my [U2 Data Analytics](https://vanleeuwen-hans.github.io/u2_data_analytics/) Quarto web site.
...
However, the docs folder is git managed in the other branch (gh-pages), so I manually copy the files to a temp folder, then switch to the gh-pages branch:

```         
git checkout gh-pages
```

and then copy the newly created docs files from the temp folder into the gh-pages docs folder:

Afterwards I do the usual add and commit, followed by a push to the remote. 
Note: if I only rendered one particular qmd file I saw that the index.html file was overwritten with a template. So I had to make sure not to add that incorrect index.html file to the gh-pages docs folder.
...
I am still trying to figure out an easier way to maintain and update the Quarto web site documents, as currently it involves a lot of manual checking and work.
